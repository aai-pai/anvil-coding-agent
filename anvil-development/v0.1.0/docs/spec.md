...
- **FR-SV-020A**: Rollback resets the active run state to the selected upstream phase, marks that phase and all downstream phases incomplete for the current run, and invalidates their checkpoint entries before re-execution begins.
- **FR-SV-020B**: Canonical outputs owned by rolled-back phases are treated as **stale** until regenerated by their owning phase agents. The supervisor may delete or rewrite only its own runtime-managed files; it must not directly rewrite phase-owned artifacts during rollback.
...
- **FR-SV-022A**: Before skipping a previously completed phase during resume, supervisor must re-validate that the checkpoint entry is readable, the referenced artifacts still exist, and stored checksums still match. If validation fails, supervisor must emit a resume-validation diagnostic and resume from the earliest invalid phase instead of skipping it.
...
#### 2.3.4 Artifact Registry and Bootstrap Ownership
- **FR-AR-007**: The central artifact manifest referenced in §2.3.1 is a **supervisor-owned logical registry**, derived from the phase artifact contracts in this Specification and from `PhaseComplete` metadata emitted by phase agents. In v0.1.0, it does not need to exist as a separate committed repository file.
- **FR-AR-008**: The **Factory Init** phase agent is the sole owner of repository bootstrap work for the canonical workspace structure (`docs/`, `src/`, `tests/`, `logs/`, and any initial `.gitkeep` placeholders). The supervisor may validate bootstrap completion, but must not author bootstrap artifacts directly.
...
- **FR-OM-008A**: Approval and override signals must include a gate identifier or target phase, requester identity (or user handle when available), decision or action, and optional comments. Override signals must include a non-empty reason.
...
- **FR-PL-003A**: Each policy rule may declare `remediable: true|false` and an optional `remediationStrategy`. If `remediable` is omitted, the default is `false`.
...
#### 2.7.4 Phase and Subtask Model Routing
- **FR-ML-001**: Anvil must support **phase + subtask hybrid model routing**. Each phase declares a default model, and individual subtasks within that phase may route to a different model when configured to do so.
- **FR-ML-002**: For v0.1.0, supported subtask categories include planning, analysis, coding, debugging, and review. Additional subtask categories may be introduced in future versions through configuration or extension updates.
- **FR-ML-003**: Default routing for v0.1.0 is:
  - Gemma 4 for planning-, analysis-, and architecture-oriented work.
  - DeepSeek Coder for coding-heavy work, including implementation-focused debugging.
- **FR-ML-004**: Model selection must remain subject to policy enforcement. If the routed default model is not permitted by `AllowedModels` or related policy, supervisor must attempt policy-compliant remediation before escalation.
- **FR-ML-005**: Users may override model routing at both the phase level and the subtask level through the standard configuration precedence hierarchy in §2.7.1.
- **FR-ML-006**: Effective model selection decisions must be logged to the audit trail with phase, subtask category (if any), selected model, and justification when an override or remediation path is applied.
...
- **FR-SK-002**: Each skill has metadata and instructions defined in a `SKILL.md` or `skill.json` file at the skill root.
...
- **FR-SA-001**: Supervisor must load specialist role registries at run start from `~/.anvil/specialist-roles.yaml` and `.anvil/specialist-roles.yaml`, merged by §2.7 precedence with workspace-local definitions overriding user-root definitions by `roleId`.
...
- **FR-SA-007**: Specialist agents may produce supplemental artifacts (for example in `docs/reviews/` or `logs/`) but must not write to protected paths outside their contract. Protected paths include canonical phase-owned outputs from §2.3.1, production source under `src/`, QA-owned test artifacts under `tests/`, and supervisor-owned runtime files under `.anvil/` and `logs/`, except for supplemental paths explicitly granted to the specialist contract.
...
- **NFR-OB-002**: Audit trail is stored in `logs/events.jsonl` (one JSON event per line) and is queryable by event type, timestamp, phase, and severity.
...
6. Full Event Log (reference to `logs/events.jsonl`)
...
#### 4.1.10 Specialist Role Registry Artifact (`~/.anvil/specialist-roles.yaml`, `.anvil/specialist-roles.yaml`)
**Purpose**: Declarative registries of optional specialist-agent roles that can be loaded and invoked by the supervisor. User-root definitions provide cross-workspace defaults; workspace-local definitions override or extend them for the active repository.
...
- When both registries define the same `roleId`, the workspace-local definition wins for the effective runtime view.
...
At runtime, canonical hook definitions are compiled into `.openhands/hooks.json`. Hook kinds that do not map 1:1 to native OpenHands callbacks must be executed by supervisor-managed adapters while preserving the same allow/deny/mutate semantics and audit logging behavior.
...
#### 4.2.3 Run Control Signal Schemas
Approval and override operations use explicit payloads:
...
```json
{
  ...
  "approved": true,
  "comments": "Architecture accepted with follow-up in blueprint",
  "requesterId": "user-or-handle"
}
```
...
```json
{
  ...
  "reason": "Need to revisit architecture assumptions",
  "comments": "Rollback requested after review",
  "requesterId": "user-or-handle"
}
```

For Secure-mode mandatory checkpoints, `approved` must be explicit and `gateName` must identify the checkpoint being accepted.
...
`remediable` defaults to `false` when omitted. If `remediable: true`, `remediationStrategy` should describe the expected auto-remediation behavior.
...
- **Location**: `logs/events.jsonl` (local to workspace)
...
| §3 Vision & Intent | Supervisor pattern, artifact contracts | FR-SV-001 through FR-SV-026, FR-AR-001 through FR-AR-008 |
...
| §4.2 v0.1.0 Success Criteria | End-to-end execution, resume, overrides, policy enforcement | FR-SV-005 through FR-SV-026, FR-OM-001 through FR-OM-014 |
...
| §8.1 Supervisor Architecture | Supervisor agent contract | FR-SV-001 through FR-SV-026 |
| §8.2 Phase Agents with Contracts | Phase agent input/output contract | FR-PA-001 through FR-PA-010 |
| §8.3 Artifact-First Workflow | 12-phase pipeline, artifact definitions | §4.1 (Artifact Schemas), FR-AR-001 through FR-AR-008 |
...
| §10 Model & Tooling Strategy | Phase-level and subtask-level model routing | FR-ML-001 through FR-ML-006 |
